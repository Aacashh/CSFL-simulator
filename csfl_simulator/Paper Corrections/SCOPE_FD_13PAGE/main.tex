...
The integration of FD with massive multiple-input multiple-output (mMIMO) wireless backbones is investigated in \cite{mu2024fedtskd}, where channel hardening and zero-forcing precoding handle the noisy uplink and downlink. The FedTSKD framework proposed there establishes a dynamic training schedule, an $\mathcal{O}(1/t)$ convergence bound, and a channel-aware aggregation rule for FD over mMIMO. It builds on the earlier wireless-FD line \cite{ahn2019wireless} and on the broader work on wireless FL over fading channels \cite{amiri2020federated, vu2020cell}.
...
\new{\textit{Privacy consideration.} Both (\ref{eq:bonus}) and (\ref{eq:penalty}) represent the per-client label histogram $\bm{h}_i$, which discloses the per-class distribution of the local dataset to the server. Two remedies are compatible with the proposed selector. The first is a calibrated Laplace mechanism on the histogram upload. The second is a server-side surrogate computed from the softmax-averaged public-dataset logits already observed in (\ref{eq:agg}), which is available once each client has been selected at least once. A quantitative characterization of either remedy on the FD pipeline is left to future investigation.}
