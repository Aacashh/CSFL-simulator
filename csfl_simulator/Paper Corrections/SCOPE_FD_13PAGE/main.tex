...
\caption{\rev{Left, effect of the Dirichlet concentration at $N=30$, $K=5$. Right, sparsity sweep at $N=30$ over three seeds. Both on FMNIST, shaded bands one standard deviation.}}
\label{fig:dirichlet}
\end{figure*}
...
\caption{\rev{Participation fairness across pool and cohort sizes on FMNIST over five seeds. Filled circles are the cycle-aligned Gini coefficient at $R=100$, open diamonds the rolling-window value over an offset window. Stars mark $K \nmid N$. Every value matches (\ref{eq:gini_exact}).}}
\label{fig:scale}
\end{figure}
...
\caption{\rev{Final accuracy relative to uniform random on FMNIST at $\alpha=0.5$, ordered by participation ratio. Each row pairs the complete score with the debt-only variant on the same seeds. Whiskers are one standard deviation of the paired difference, over five seeds except at $30/1$, $30/3$ and $30/10$, where three are used.}}
\label{fig:ratio}
\end{figure}
