...
\noindent
\textbf{Reply 2:} \PENDING{The Laplace-mechanism sweep over $\varepsilon$ and the server-side surrogate-histogram variant are currently running. This reply will report the accuracy and fairness cost of each variant and will state the resulting limitation.}\\
...
\noindent
\textbf{Reply 5:} \PENDING{The channel-aware and energy-aware variant is currently running under an energy-budget-constrained scenario. This reply will report the measured effect and will then either present the variant or adopt the positioning that the Reviewer suggests.}\\
...
\noindent
\textbf{Reply 5:} \PENDING{The client-dropout experiment and the bounded-staleness asynchronous experiment are currently running. This reply will report how the fairness guarantee and the accuracy behave when selected clients fail to return logits and when logits arrive late.}\\
...
\noindent
\textbf{Reply 6:} \PENDING{The public-dataset sensitivity study is currently running. It varies the identity of the public set, its size, and the level of injected label noise. This reply will report those results.}\\
