...
\noindent
\textbf{Comment 3:} \textit{While efficiency improvements are strong, the accuracy gains are relatively modest (~1–1.5\%).
Please better justify:
Why these gains are meaningful in practice?
Scenarios where efficiency is prioritized over accuracy.} \\
\noindent
...
\textbf{Comment 1:} \textit{The manuscript should more clearly articulate the exact novelty of MAML-Select compared with existing adaptive client selection methods such as RL-based approaches, contextual bandits, and meta-learning-based FL personalization frameworks. A dedicated comparison table highlighting differences in objective, adaptation mechanism, computational complexity, and optimization strategy would improve clarity.} \\
\noindent
...
\textbf{Comment 2:} \textit{The manuscript provides empirical evaluation only. There is no convergence proof or theoretical justification of why the meta-learning formulation improves non-stationary adaptation.}\\
\noindent
...
\textbf{Comment 3:} \textit{Equation (4) combines latency penalty and accuracy gain using a manually chosen trade-off parameter λ, but there is no explanation of how λ was selected or how sensitive the framework is to this parameter.} \\
\noindent
...
\textbf{Comment 4:} \textit{The experimental validation is limited to small-scale datasets. Additional experiments on larger and more realistic FL benchmarks would strengthen the claims regarding scalability and practical applicability.} \\
\noindent
...
\textbf{Comment 5:} \textit{ The reported performance improvements are relatively modest. The manuscript should include standard deviation across multiple runs and statistical significance analysis to demonstrate robustness and reproducibility.} \\
\noindent
...
\textbf{Comment 6:} \textit{Since the framework incorporates latency-aware optimization, the manuscript should evaluate fairness across heterogeneous clients. Metrics such as participation fairness, client coverage, or performance across client tiers would provide valuable insight.}\\
\noindent
...
\textbf{Comment 7:} \textit{The claims regarding energy efficiency and Green AI are currently indirect. The manuscript estimates computational savings through TFLOPS but does not report actual energy consumption, power measurements, or carbon footprint analysis. The wording should be moderated or supported with stronger evidence.} \\
\noindent
\textbf{Reply 7:} {We thank the Reviewer for the insightful comment. In the revised manuscript, we
...
and the local-training time. Hence no Green-AI claim is made and we report only modelled energy and computational efficiency.}

\noindent

\textbf{Comment 8:} \textit{The manuscript should justify the choice of the six state features used in the meta-policy. An ablation study evaluating the contribution of each feature would improve interpretability.} \\
\noindent
\textbf{Reply 8:} {We thank the Reviewer for motivating us to provide the ablation study.
...
accuracy slightly, but it lowers the Jain's index the most, from $0.78$
...
in the manuscript towards six features: The server constructs
...
\end{center}
\noindent{Change in final accuracy on Fashion-MNIST when each feature is removed in turn. Removing the battery feature causes the largest drop, confirming that both learning and system signals are used.}

\noindent
\textbf{Comment 9:} \textit{Fig. 2 has small font sizes and unclear
axis scaling.} \\
\noindent
\textbf{Reply 9:} {We thank the Reviewer for comment. In the revised manuscript, we have improved the font sizes of all the figures. Axis are also defined clearly with their
symbols and units. The convergence figure (Fig.~2) in the revised manuscript is now at full column width with proper font size, resulting in improved figure quality.  }
%Specifically, the figure text now matches the manuscript body text in both font family and size, neither larger nor smaller.
\noindent
...
is removed. The results table (Table~II) now reports only well-defined quantities. These are accuracy, weighted precision, recall, F1, TFLOPs, modelled energy (Wh), Jain's index and coverage.}

\noindent

\textbf{Comment 11:} \textit{The manuscript discusses several recent approaches such as CriticalFL and FedGCS but does not include them in the experimental comparison. Including these baselines would provide a more convincing evaluation.} \\
\noindent
\textbf{Reply 11:} {We thank the Reviewer for suggesting
the two baselines. In the revised manuscript, we have now added the
comparison with both the suggested methods. Both are also now in the comparison Table-I, the
...
in total, that is, FedAvg, FedCS, Oort, TiFL, FedCor, FedGCS, CriticalFL and the proposed MAML-Select. Their client selection strategies span over a wide variety, including random, system-aware, utility-aware, correlation-based, generative, and critical-period selection. Therefore, the evaluation now places MAML-Select against current state-of-the-art selectors.}
...
\noindent
\textbf{Reply 12:} {Thanks for the comment. In the revised manuscript, all the grammatical, punctuation and sentence structure errors have been removed at our best knowledge.}\\

\hspace{0.75cm}{\bf {\Large {~~~~~~~~~~~~Response to Comments of Reviewer 3}}}

\noindent
\textbf{Comment 1:} \textit{The motivation of this work is not clear to the reader.} \\
\noindent
\textbf{Reply 1:} {We thank the Reviewer for pointing this out. In the revised version, the manuscript is significantly
...
As per the Reviewer's suggestion, the above discussed motivation is now updated in the Abstract and the Introduction section of the revised manuscript.}

\noindent
\textbf{Comment 2:} \textit{The authors must focus on the novelty issues and highlight the main contribution so that it becomes easier for the readers to get the main points of this paper.} 

\noindent
\textbf{Reply 2:} {We thank the Reviewer for the valuable suggestion.
In the revised manuscript, we have stated
the contribution more precisely and clearly in the introduction section.
%To the best of authors' knowledge, the proposed method is the first one to treat per-round federated client selection as a meta-learning task, and to solve it with first-order MAML. Among the novelty and contribution, the following are main contributing factors. First, the meta-learned object is the server-side ranking policy, not the client model. Thus, this MAML model is not for personalization of clients model. Second, each round is treated as a task and has an explicit support set, which is the previous round's selected-client feedback. Each round also has a query set, which is the current candidate pool. Thus, the policy is re-fitted by a gradient step right before every selection. Third, the cost that supervises the policy joins learning progress and system latency using the weight parameter, $\lambda$, which enables a tunable computational and energy efficiency and accuracy operating point. 
The following has been updated in the revised manuscript:

The major contributions can be summarized as follows.

1) We formulate the FL client selection problem as a meta-learning
task and introduce the MAML-Select algorithm with rapid gradient-based adaptation for online client selection.

2) Experiments on Fashion-MNIST, CIFAR-10, and CIFAR-100 under
...
the state features support the design choices.

3) We demonstrate that MAML-Select lowers cumulative training
...
\noindent
\textbf{Comment 3:} \textit{Did you consider any constraints for the problem formulation? If so, please update the formulated problem with appropriate constraints. } 

\noindent
\textbf{Reply 3:} {We thank the Reviewer for pointing out
the constraints in the problem formulation. In Equation (3) in
the section III of the revised manuscript, we have added the missing/implicit
constraints, and the selection problem is fully specified as 
\begin{equation}
\nonumber
...
$\mathcal{A}_{t}$. It adds binary decision variables $a_{i,t}\in\{0,1\}$, the cohort-size constraint $\sum_{i}a_{i,t}=K$, and the
...
\noindent
\textbf{Comment 4:} \textit{Why is only one method considered as a baseline? The authors should consider at least two more baselines.} 

\noindent
\textbf{Reply 4:} {We thank the Reviewer for the insightful comment.
In the revised manuscript, the evaluation now compares the
proposed MAML-Select against seven other baselines algorithms. All of these existing methods appear in Table~I of the revised manuscript and corresponding evaluation is presented in the Evaluation and Results section of the revised manuscript. In particular, please refer Figs. 2 and 3 of the revised manuscript. Therefore, the comparison is now broad, not single-baseline.}

%FedAvg has random client selection. FedCS is system-aware and deadline-constrained client selection. The client choosing in the Oort uses utility-based importance sampling. TiFL adopts a tier-based approach to trim the clients. FedCor employs correlation-based client filtering, using Gaussian Processes. FedGCS utilizes generative methods to include and exclude clients. CriticalFL uses critical-period cohort scaling. 


\noindent
\textbf{Comment 5:} \textit{More simulation results need to be provided in the paper to ensure the effectiveness of the proposed method.}

\noindent
\textbf{Reply 5:} {We thank the Reviewer for asking to provide more experimental analysis. As per your suggestions, we have performed various experiments on different datasets using more number of baseline methods. The revised manuscript covers three datasets and seven other 
baseline methods. Fashion-MNIST and CIFAR-10 run for $200$ rounds, while CIFAR-100
runs for $150$ rounds. The new simulations covers eight methods and
three seeds, with mean\,$\pm$\,std and paired-test significance. Please refer the Evaluation and Results section of the revised manuscript.
%Fairness and coverage metrics are also added. A two-dataset $\lambda$ sweep is also added. Energy in terms of computational overhead is added. A supporting complexity result is added to provide selector-overhead scaling study. The reported metrics include accuracy, precision, recall, F1, compute, energy, fairness, coverage, sensitivity, ablation, and server-side overhead. 

%More tables and figures are in the Supplementary Material (Sections~S2 to~S5).}

\noindent
\textbf{Comment 6:} \textit{Remove the references from the conclusion and only add a single sentence as future work. The authors are advised to write the conclusion in a single paragraph.} 

\noindent
\textbf{Reply 6:} {We thank the Reviewer for the insightful comment and suggestion. The Conclusion section is now a single concise paragraph
with no citations. It ends with a short statement of future directions.}

\hspace{0.75cm}{\bf {\Large {~~~~~~~~~~~~Response to Comments of Reviewer 4}}}

\noindent
\textbf{Comment 1:} \textit{There are some English and Grammatical mistakes. Therefore, improve and proofread your article thoroughly and carefully.  
I. In Impact statement: This work addresses the challenge of deploying FL in heterogeneous edge environments such as Internet of Vehicles (IoV) and mobile healthcare systems. Should be This work addresses the challenge of deploying FL in heterogeneous environments, such as Internet of Vehicles (IoV) and mobile healthcare systems.
II. In Impact statement: The broader impact of this work lies in enabling scalable, adaptive, and energy-efficient distributed learning systems, which are critical for next-generation applications, including smart cities, autonomous systems, healthcare monitoring, and industrial IoT, should be The broader impact of this work lies in enabling scalable, adaptive, and energy-efficient distributed learning systems, which are critical for next-generation applications, including smart cities, autonomous systems, healthcare monitoring, and industrial Internet of Things.}\\
\noindent
\textbf{Reply 1:} We thank the Reviewer for asking to perform a careful proofreading. Both corrections have been incorporated in the revised Impact Statement. The phrase ``heterogeneous edge environments" has been simplified to ``heterogeneous environments," as suggested, and ``industrial IoT" has been expanded to the full form "industrial Internet of Things" for terminological consistency throughout the manuscript. We have also conducted a thorough proofread of the entire paper to address any remaining grammatical issues and to ensure consistent use of terminology.

\noindent
\textbf{Comment 2:} \textit{The sentences throughout this paper are very long. Therefore, improve and proofread your article thoroughly and carefully by making them a reasonable length.} \\
\noindent
\textbf{Reply 2:} {We thank the Reviewer for this important observation. The revised manuscript has been extensively edited to reduce sentence length and improve overall readability. Compound sentences that combined multiple independent clauses have been split into shorter and clearer statements. Parenthetical qualifications have been moved to separate sentences or to footnotes where appropriate. The revised paper has been proofread multiple times by all the authors to ensure consistent readability improvements throughout. We believe the revised version is substantially more accessible than the original submission.}

\noindent
\textbf{Comment 3:} \textit{Mostly papers cited in literature review section are outdated. Update and revise your Literature review by including more recent and related papers.} \\
\noindent
...
\noindent
\textbf{Comment 4:} \textit{I would suggest including a comparison table with previous state-of-the-art works to prove and show the novelty of your proposed framework.}\\
\noindent
\textbf{Reply 4:} {We thank the Reviewer for the suggestion.
In the revised manuscript, we have added the Table~I, which compares
...
\noindent
\textbf{Comment 5:} \textit{The future work mentioned in Conclusion and Future Work Section should be more accurate and concise and try to mention in some better style rather than mentioning and giving to many citations.}\\
\noindent
\textbf{Reply 5:} {We thank the Reviewer for the comment.
The Conclusion section is now revised to a single concise paragraph without citations. The section ends with a single statement of future directions.}

\noindent
\textbf{Comment 6:} \textit{Figure 2 required more clarifications and justifications as well as use clearer figure that highlight difference with the other schemes.}\\
\noindent
\textbf{Reply 6:} {We thank the Reviewer for this comment and suggestion. In the revised manuscript, Fig.
2 (the original single-curve figure in the old manuscript) has been
replaced by clearer and multi-metric evidence. Fig. 2 now reports
CIFAR-100 accuracy, the selector meta-update magnitude (policy stabilization), cumulative modeled energy, and testing loss over rounds; the accuracy,
energy, and loss panels cover all eight methods. Additionally, Fig. 3(a) has been introduced as a complementary efficiency–accuracy trade-off plot across all three datasets. This figure makes the position of MAML-Select immediately visible. MAML-Select consistently occupies the lower-cost region (x < 1) while remaining close to the top-accuracy cluster. We believe the combination of the revised Fig. 2 and the new Fig. 3(a) provides substantially clearer and more complete comparative evidence than the original single-curve figure.\\


\textbf{Once again, we would like to express our gratitude for your insightful comments and the
time you dedicated to reviewing our paper. We hope that the newly incorporated changes have successfully
addressed your suggestion and improved the overall quality of our submission.}\\
