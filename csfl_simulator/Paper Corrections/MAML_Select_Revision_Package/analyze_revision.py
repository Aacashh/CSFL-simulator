...
def load_runs(root: str) -> List[Run]:
    runs: List[Run] = []
    for base in ("MAML-Revision-2", "runs", "csfl_simulator"):
...
def selector_convergence(root: str) -> dict:
    ...
    # run_maml_revision2.sh writes these under csfl_simulator/runs/, while the
    # original single run was kept beside the manuscript. Look in both, newest
    # first, so the analysis picks up a fresh Phase A on any machine.
    bases = [
        os.path.join(root, "MAML-Revision-2", "convergence"),
        os.path.join(root, "csfl_simulator", "runs", "maml_select_convergence"),
        os.path.join(root, "csfl_simulator", "Paper Corrections", "maml_select_convergence"),
    ]
    out = {}
    for name in ("fashion", "cifar10", "cifar100"):
        candidates = [os.path.join(b, f"selector_convergence_{name}.jsonl") for b in bases]
        ...
        drift = [r["drift_increment"] for r in rows
                 if r.get("drift_increment") is not None
                 and r["drift_increment"] == r["drift_increment"]]
        out[name]["drift_rounds"] = len(drift)
        out[name]["drift_mean"] = round(st.fmean(drift), 6) if drift else None
        out[name]["drift_sum"] = round(sum(drift), 4) if drift else None
        out[name]["drift_abs_sum"] = round(sum(abs(d) for d in drift), 4) if drift else None
        out[name]["source"] = os.path.relpath(path, root)
    return out
