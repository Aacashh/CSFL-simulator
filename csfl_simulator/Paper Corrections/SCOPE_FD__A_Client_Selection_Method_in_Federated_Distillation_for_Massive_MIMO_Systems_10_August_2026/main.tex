...
\rev{Two questions are settled at the headline configuration $N=30$, $K=5$, $\alpha=0.5$ over five seeds. The first is how much each of the three score terms contributes, which is answered by the four variants in the upper block of Table~\ref{tab:headline}. The second is how the selector compares against published methods ported into the FD pipeline as described in Section~\ref{ssec:rw-fl-selection}, which is the lower block. The complete score returns $71.21 \pm 0.99$ in both experiment families, so the two blocks share a common anchor.}

\begin{table}[t]
\centering
\caption{\rev{Score-term ablation (upper) and selector comparison (lower). FMNIST, $N=30$, $K=5$, $\alpha=0.5$, five seeds, mean $\pm$ standard deviation.}}
\label{tab:headline}
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Selector} & \textbf{Accuracy (\%)} & \textbf{Gini (\%)} & \textbf{Rounds to $70\%$} \\
\midrule
...
\midrule
UnionFL \cite{castillo2025subtrunc}  & $71.18 \pm 1.40$ & $1.33$  & $71.0$ \\
Uniform random      & $70.99 \pm 1.38$ & $12.49$ & $74.8$ \\
DivFL \cite{balakrishnan2022divfl}   & $69.88 \pm 0.83$ & $36.84$ & $76.0$ \\
SubTrunc \cite{castillo2024subtrunc} & $69.40 \pm 1.00$ & $38.65$ & $86.0$ \\
Oort \cite{lai2021oort}              & $21.14 \pm 0.60$ & $83.33$ & --- \\
\bottomrule
\end{tabular}
\end{table}

\rev{The participation-debt term supplies the entire fairness result. All four variants reach a Gini coefficient of $1.33\%$, the value (\ref{eq:gini_exact}) specifies for this configuration, and removing either information term leaves it unchanged, exactly as Proposition~1 predicts since those terms act only inside a debt-equal cohort. The four are also statistically indistinguishable in final accuracy, with the standard deviations overlapping in every pair. The only measurable effect of the two information terms is on convergence speed, where the complete score reaches $70\%$ absolute accuracy in $69.8$ rounds against $71.0$ for the debt term alone, a margin small relative to its spread across seeds. The fairness guarantee is therefore the primary result and the information terms refine the convergence path rather than the final accuracy. Repeating the comparison across $K \in \{1, 3, 5, 10\}$ leaves the spread between variants between $0.46$ and $1.58$ percentage points at every $K$ with no variant leading consistently, so the conclusion is not specific to the headline configuration.}
...
\rev{The fairness guarantee is finally stressed along the three directions where its derivation is least immediate, namely a larger client pool, a cohort size that does not divide the pool, and an evaluation window that does not align with a participation cycle. Configurations with $N$ up to $200$ and $K$ up to $40$ are used, including the non-divisible cases $N=47$ with $K=6$ and $N=53$ with $K=7$. The largest cohort occupies $40$ of the $N_{BS}=64$ spatial dimensions the array can separate, so the sweep covers most of the range the physical layer admits under (\ref{eq:ul_zf}). Alongside the cycle-aligned Gini coefficient, a rolling-window Gini coefficient is computed over a window deliberately offset from the cycle length $\lceil N/K \rceil$. Fig.~\ref{fig:scale} shows both.}

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_r6_scale_fairness}
\caption{\rev{Participation fairness across client-pool and cohort sizes on FMNIST over five seeds, one bar per selector. Filled circles are the cycle-aligned Gini coefficient at $R = 100$; open diamonds the rolling-window coefficient over a window offset from the participation cycle, so bar length shows how much the reading depends on window placement. Stars mark $K \nmid N$. Every value matches (\ref{eq:gini_exact}).}}
\label{fig:scale}
\end{figure}
...
\caption{\rev{Final accuracy relative to uniform random selection on FMNIST at $\alpha = 0.5$, for every matched configuration, ordered by participation ratio. Each row pairs the complete score with the debt-only variant on the same seeds, the grey connector being the distance between them. Whiskers are one standard deviation of the paired difference over seeds, five per configuration except at $30/1$, $30/3$ and $30/10$, where three are used.}}
\label{fig:ratio}
\end{figure}
...
\rev{The test is direct. For every run, $N$, $K$ and $R$ are known before the run starts, so (\ref{eq:gini_exact}) fixes the participation Gini coefficient in advance. Across the campaign it is correct in $459$ of $459$ cases, covering $276$ runs of the proposed selector and $183$ of the debt-only variant, with no deviation at all rather than a small one. Held out from that count are the eighteen dropout runs, where a selected client may fail to return and the realised participation is therefore not the intended participation. The measured value is also independent of the seed, with a standard deviation of exactly zero over seeds at every configuration, which is what a deterministic rule should give.}

\rev{The law holds across every condition reported in Sections~\ref{ssec:exp-coefgrid} to~\ref{ssec:exp-cross-dataset}, that is across all three datasets, all seven heterogeneity levels, the whole SNR sweep, every privacy mechanism, both staleness windows, every public-set variation and all thirty cells of the coefficient grid. The value $1.33\%$ is returned in $313$ of these runs, spanning fifteen of the seventeen families, because those runs share the headline arithmetic, and every remaining run returns the value the law specifies for its own triple.}
