...
\begin{enumerate}
    ...
    \item \textit{Convergence Compatibility with FedTSKD.} We show that the proposed selector modifies only the composition of the per-round client subset, leaving the local training, distillation, and server aggregation steps of FedTSKD \cite{mu2024fedtskd} unchanged. \rev{The selector is therefore compatible with the $\mathcal{O}(1/t)$ analysis of \cite{mu2024fedtskd} under the assumptions of that work, and we set out which of those assumptions a deterministic without-replacement rotation satisfies rather than claiming the bound transfers unaltered.}

    \item \textit{Experimental Analysis.} \rev{We report every quantity as a mean with a standard deviation over independent seeds, on Fashion-MNIST with Dirichlet non-IID partitions and on MNIST and a spoken-digit corpus, sweeping the cohort size $K$, the Dirichlet concentration $\alpha$, the downlink signal-to-noise ratio, the client-pool size $N$ and the two selector coefficients. The selector lowers the participation Gini coefficient by roughly an order of magnitude against uniform random in every tested configuration while matching its accuracy. A four-way ablation separates the contribution of each score term, and four published selectors ported into the FD pipeline serve as comparison points.}
\end{enumerate}
...
\rev{Three consequences follow, each checked against measurement in Section~\ref{ssec:exp-invariance}. First, the bound (\ref{eq:gini_bound}) is tight rather than merely sufficient, since $m(N-m)$ is maximised at $m = N/2$, where it recovers (\ref{eq:gini_bound}) with equality for even $N$. Second, the coefficient vanishes exactly when $N$ divides $KR$, a condition on the horizon as well as on the pool and the cohort, so the divisibility of $N$ by $K$ is neither necessary nor sufficient. At the headline setting $N=30$, $K=5$, $R=100$ the cohort divides the pool yet $KR = 500$ leaves a remainder of $20$ and the coefficient settles at $1.33\%$, while at $N=50$ with $K=3$ the cohort does not divide the pool yet $KR = 300$ is an exact multiple of $50$ and the coefficient is zero. Third, the decay in $R$ is not smooth. Its envelope falls as $\mathcal{O}(1/R)$ while the value oscillates with $m$, touching zero whenever the horizon completes a whole number of rotations.}

\rev{Equation (\ref{eq:gini_exact}) also isolates what the guarantee can depend on. Its right-hand side contains only $N$, $K$ and $R$, so no term of the score, no property of the data and no property of the channel enters it. Any perturbation leaving those three fixed cannot move the coefficient, provided the selected clients do participate. The divisibility condition $K \mid N$ is needed only for the per-window statement, in which each cycle of $\lceil N/K \rceil$ rounds serves every client once. The bound (\ref{eq:spread}) is proved without it, and it holds at every round rather than only at cycle ends, so neither $K \nmid N$ nor an offset evaluation window breaks the guarantee. Both cases are examined in Section~\ref{ssec:exp-scale} and the campaign-wide test is reported in Section~\ref{ssec:exp-invariance}.}
...
\rev{Identity has the largest effect. Replacing the held-out public set with MNIST lowers the accuracy of the proposed selector from $71.75 \pm 0.81$ to $67.10 \pm 1.05$, a drop of $4.65$ percentage points, which reflects the mismatch between the public inputs on which the consensus logits are formed and the private inputs on which the clients train. This cell also serves a second purpose. Because MNIST is a separate corpus, its inputs are disjoint from the Fashion-MNIST evaluation data, whereas the default public set is drawn from the held-out split of the private dataset itself. The ordering of the selectors is unchanged under the disjoint public set, with the proposed selector at $67.10 \pm 1.05$, the debt-only variant at $67.14 \pm 0.95$ and uniform random at $66.72 \pm 0.89$. The absolute accuracy level does depend on how the public set is chosen, but the comparison between selectors does not.}
...
\rev{This is a real boundary of the guarantee rather than an implementation defect. Proposition~1 assumes every client remains selectable so the debt ordering can be honoured, and a filter that removes clients on a criterion uncorrelated with debt breaks that assumption. Starving rounds of clients also thins each consensus logit, which accounts for the accuracy deficit. The proposed selector is therefore positioned as a fairness and data-coverage selector running on top of an mMIMO substrate rather than one that exploits channel state.}
...
\rev{Across the three datasets the pattern is the same. The fairness separation is identical everywhere, because it depends only on the participation arithmetic. The accuracy ordering is also preserved, with the proposed selector and the debt-only variant tied with each other and ahead of both submodular methods. What changes across datasets is the absolute accuracy level, which reflects the difficulty of the task rather than the selection rule.}

\subsection{Invariance of the Fairness Guarantee}
\label{ssec:exp-invariance}

\rev{Taken together the experiments above support a stronger statement than that the proposed selector is fair. The campaign comprises $284$ completed runs across seventeen experiment families, and it is large enough to test (\ref{eq:gini_exact}) as a prediction rather than to report fairness one setting at a time.}
...
\rev{The law holds across every condition reported in Sections~\ref{ssec:exp-coefgrid} to~\ref{ssec:exp-cross-dataset}, that is across all three datasets, all seven heterogeneity levels, the whole SNR sweep, every privacy mechanism, both staleness windows, every public-set variation and all thirty cells of the coefficient grid. The value $1.33\%$ is returned in $202$ runs spanning fifteen of the seventeen families, because those runs share the headline arithmetic. The rest change $N$, $K$ or $R$, and each returns the value the law specifies for its own triple.}

\rev{This follows from the structure of the score. The right-hand side of (\ref{eq:gini_exact}) contains only $N$, $K$ and $R$, because the participation-debt term reads participation counts alone, so no perturbation that leaves those three fixed can move the coefficient whatever it does to the data, the channel, the public set or the privacy mechanism. The practical reading is that fairness can be predicted from the deployment parameters instead of being re-validated for each deployment condition, which is what a penalty-based or stochastic fairness objective cannot offer. The one condition under which it degrades, a hard resource constraint that renders clients unselectable, is characterised in Section~\ref{ssec:exp-channel-aware}. Dropout is the milder form of the same effect, and the coefficient then varies from run to run between $0.42\%$ and $2.12\%$ rather than taking the single value the law specifies.}
...
\new{Four extensions are left open. The first is to admit channel and energy signals into the score while preserving the participation guarantee, which the energy-budget result shows is not straightforward. \rev{The second is a convergence analysis for deterministic partial participation that exploits the without-replacement structure of the rotation, rather than the compatibility argument used here.} The third is a ratio-dependent weighting of the two information terms, since Section~\ref{ssec:exp-scale} locates their cost at high participation ratios. \rev{The fourth is a wider study of non-image domains, on corpora large enough for absolute accuracy to be informative, which the spoken-digit corpus used here is not.}}
\balance
% \IEEEtriggeratref{19}
