...
\begin{abstract}
\rev{Federated distillation lets edge devices train a shared model by exchanging only their predictions on a public dataset, which costs far less over a wireless link than sending weights. A base station can serve only a few of its devices per round, so it must choose which ones. Client selection has been studied at length for federated learning, where devices are ranked by a utility signal such as local loss or gradient magnitude. Federated distillation averages predictions rather than weights, which flattens those per-device signals, so the same rules do not carry over. Published federated distillation systems for massive MIMO networks avoid the question by assuming that every device participates in every round. No selector for this setting can state how evenly participation will be spread, and choosing at random leaves some devices served many times more often than others. This paper proposes SCOPE-FD, a deterministic selector that rotates devices by how far each has fallen behind a uniform participation target, and breaks ties towards devices holding classes the server currently predicts poorly. The participation imbalance it produces depends only on the pool size, the number served per round, and the training horizon, so it can be calculated before training starts. That prediction held in every run of a large experimental study, while accuracy matched the random baseline. Participation fairness is therefore constructible, not merely achievable, which turns a property machine learning usually imposes as a penalty and verifies afterwards into one an operator can commit to in advance.}
...
\begin{abstract}
\rev{Wireless networks are beginning to train shared artificial intelligence models across phones, wearables and vehicles without gathering anyone's data centrally. Only a handful of devices can be served in each training round. Picking them at random means some devices contribute many times more than others, so whose data shapes the resulting model becomes partly a matter of chance. This work replaces that lottery with a rotation whose fairness follows from three quantities a network operator already knows, and which can therefore be stated before a deployment is switched on rather than measured after it has run. Operators give up no accuracy in exchange. As artificial intelligence systems come under growing legal obligation to evidence equitable treatment of the people they serve, the ability to commit to a participation guarantee in advance, rather than report one in hindsight, changes what such a deployment can promise both its users and its regulators.}
\end{abstract}}
\vspace{-1em}
\section{Introduction}
\label{sec:introduction}
\IEEEPARstart{F}{EDERATED} Learning (FL) \cite{borazjani2026redefining} lets a population of devices train a shared model under the coordination of a central server without their data ever leaving them, a property studied at length in \cite{rao2025privacy, lim2020federated, huang2024survey, lu2024noniid}. In each communication round the selected clients train locally on their own data and upload the updated model parameters for aggregation. Uploading full model weights does not scale to modern architectures over a wireless edge, however, particularly when many devices share the same connectivity. Federated Distillation (FD) \cite{xing2022efficient} avoids that cost by having each client upload only its output logits on a shared public dataset \cite{itahara2023distillation}. Such a logit-only exchange is able to reduce the per-round communication overhead substantially \cite{sattler2022cfd}, \cite{sattler2023fedaux}, and permits clients to train heterogeneous local architectures, while further improving privacy \cite{chan2021fedhe, cho2023clustered, wu2024fedict}.
...
In this paper, we propose SCOPE-FD (Server-aware Coverage with Over-round Participation Equalization), a deterministic client selection algorithm for FD over mMIMO networks, built on top of the FedTSKD of \cite{mu2024fedtskd}. Existing FL selectors \cite{nishio2019client}, \cite{cho2020bandit}, \cite{xia2020mab}, \cite{balakrishnan2022divfl} score clients either by utility or by coverage but not both, while existing FD work \cite{mu2024fedtskd}, \cite{itahara2023distillation}, \cite{liu2022active} either assumes full participation or addresses partial participation only along the data-sampling direction. The proposed algorithm instead composes participation balance, server-side class under-prediction and per-round class coverage in one deterministic score \revIVE{with non-negative coefficients held fixed across all configurations}. To the best of the authors' knowledge it is the first deterministic, fairness-guaranteed client selector tailored to FD's data-size-weighted logit-averaging aggregation rule \cite{itahara2023distillation}, \cite{mu2024fedtskd} under partial participation over an mMIMO backhaul. The main contributions are summarized as follows.

\begin{enumerate}
    \item \textit{Three-Term Deterministic Selector for FD.} We design a selector combining a participation-debt rotation term, a server-side per-class under-prediction bonus and a per-round class-coverage penalty, with the debt term as the primary signal and the other two acting as tie-breakers inside debt-equal cohorts. It carries zero trainable parameters, unlike the reinforcement-learning and neural-bandit FL selectors \rev{\cite{kazemi2026robust}, \cite{xia2020mab}} that maintain $\mathcal{O}(N\cdot K)$ trainable states, and fills $K$ slots in $\mathcal{O}(KNC)$ time per round for $C$ label classes.

    \item \textit{Participation-Fairness Guarantee.} We prove that, under the proposed selector, the per-client cumulative participation counts differ by at most one at every round. When $K$ divides $N$, this sharpens to exact equality at the end of every $\lceil N/K \rceil$-round cycle, so the participation Gini coefficient over each such window is identically zero. \rev{We further show that the cumulative coefficient after $R$ rounds is not merely bounded but exactly $m(N-m)/(NKR)$ with $m = KR \bmod N$, which makes the $\mathcal{O}(1/R)$ bound tight, identifies $N \mid KR$ as the exact condition for it to vanish, and lets the fairness of a deployment be computed in advance.}

    \item \textit{\rev{Round-Indexed Convergence under Partial Participation.}} We show that the proposed selector modifies only the composition of the per-round client subset, leaving the local training, distillation, and server aggregation steps of FedTSKD \cite{mu2024fedtskd} unchanged. \rev{Composing the per-client bound of \cite{mu2024fedtskd} with the participation guarantee above gives a rate of $\mathcal{O}(N/(KR))$ in communication rounds that holds for every client at every finite horizon rather than on average. It asks for nothing beyond the assumptions of \cite{mu2024fedtskd}, since the analysis there is carried out under full participation and has no client-sampling step, and we work through those assumptions one at a time. The rate returns the $\mathcal{O}(1/R)$ result of \cite{mu2024fedtskd} at full participation.}

    \item \textit{Experimental Analysis.} \rev{We report every quantity as a mean with a standard deviation over independent seeds, on Fashion-MNIST with Dirichlet non-IID partitions and on MNIST and a spoken-digit corpus, sweeping the cohort size $K$, the Dirichlet concentration $\alpha$, the downlink signal-to-noise ratio, the client-pool size $N$ and the two selector coefficients. The selector lowers the participation Gini coefficient by roughly an order of magnitude against uniform random in every tested configuration. On accuracy it leads where participation is sparse, ties at the headline setting and trails once the cohort reaches twenty clients. A four-way ablation separates the contribution of each score term, and four published selectors ported into the FD pipeline serve as comparison points.}
\end{enumerate}
...
This section presents the theoretical analysis of SCOPE-FD. The participation-balance guarantee is established first. \rev{It is then composed with the per-client convergence bound of FedTSKD \cite{mu2024fedtskd} to give a rate in communication rounds under partial participation.}
...
\rev{The divisibility condition $K \mid N$ is needed only for the per-window statement, in which each cycle of $\lceil N/K \rceil$ rounds serves every client once. The bound (\ref{eq:spread}) is proved without it and holds at every round rather than only at cycle ends, so neither $K \nmid N$ nor an offset evaluation window breaks the guarantee. Both cases are examined in Section~\ref{ssec:exp-scale}.}
...
\rev{\textit{Proof.} Only the clients in $\mathcal{S}_r$ update in round $r$, so the clock of client $n$ advances by $E_r + E_d \ge E_{\min} + E_d$ steps on each round where it is selected and by nothing on the others. After $R$ rounds it stands at $E_{\min}+E_d$ times that client's participation count or above, and the count form behind (\ref{eq:gini_exact}) puts every count at $\lfloor KR/N \rfloor \ge KR/N - 1$. Substituting both facts into (\ref{eq:conv_bound}) gives the claim. \hfill$\blacksquare$}
...
\rev{\textit{Proof.} Only $\nu_n$ depends on $n$ in (\ref{eq:cor_perclient}), since $\Gamma_R$ does not, so weighting by $\lambda_n$ and summing gives (\ref{eq:cor_global}). The objective (\ref{eq:global_obj}) is the $\lambda_n$-weighted sum of the local losses and \cite{mu2024fedtskd} poses $N$ separate per-client minimizations, so $F^{*} = \sum_n \lambda_n L_n^{*}$. \hfill$\blacksquare$}
...
\rev{\textit{Reporting protocol.} Every configuration is repeated over independent seeds and every reported quantity is a mean with a standard deviation over those repetitions. The headline configuration and the selector comparison use five seeds and the parameter sweeps use three, as stated in each table. Accuracy comparisons between two selectors are supported by a paired Wilcoxon signed-rank test over matched seeds, so the data partition and the channel realization are held fixed within each pair. Convergence speed is reported as the number of rounds needed to reach a fixed absolute accuracy threshold, since a threshold relative to each method's own final accuracy is not comparable across methods that converge to different ceilings.} Each client holds a private partition with Dirichlet concentration $\alpha = 0.5$, and clients adopt heterogeneous local architectures from the FD-CNN1/2/3 pool \cite{chan2021fedhe}. \rev{The public set used for logit exchange \cite{itahara2023distillation} is $2000$ unlabeled samples drawn once from the held-out split of the private dataset under a fixed sampling seed. Only the inputs are used, since the aggregation rule (\ref{eq:agg}) consumes public-set logits and never reads a public-set label. Section~\ref{ssec:exp-publicset} treats the public set as a variable.} The mMIMO BS is equipped with $N_{BS} = 64$ antennas, the uplink SNR is fixed at $-8$~dB, and the downlink SNR is varied over $\{\text{error-free}, 0, -10, -20, -30\}$~dB. The distillation temperature is fixed at $T = 1.0$ to match the underlying FedTSKD framework \cite{mu2024fedtskd}.
...
FMNIST is the primary benchmark because it offers FD-signal headroom for the selection rule to be observable across both sweeps. \rev{Two further datasets test whether the conclusions carry beyond it, namely MNIST and the Free Spoken Digit Dataset, reported in Section~\ref{ssec:exp-cross-dataset}.}
...
Private datasets & \rev{FMNIST (primary), MNIST, FSDD} \\
...
\subsection{Ablation of the Score Terms}
\label{ssec:exp-ablation}

\rev{The score (\ref{eq:score}) combines three terms, so the first question is how much each one contributes. Four variants are compared at the headline configuration $N=30$, $K=5$, $\alpha=0.5$ over five seeds. The variants are the participation debt alone, the debt with the under-prediction bonus, the debt with the class-coverage penalty, and the complete score. The results are listed in Table~\ref{tab:ablation4}.}

\begin{table}[t]
\centering
\caption{\rev{Ablation of the score terms. FMNIST, $N=30$, $K=5$, $\alpha=0.5$, five seeds, mean $\pm$ standard deviation.}}
\label{tab:ablation4}
...
\rev{Two observations follow. First, the participation-debt term supplies the entire fairness result. All four variants reach a Gini coefficient of $1.33\%$, the value (\ref{eq:gini_exact}) specifies for this configuration, and removing either information term leaves it unchanged, exactly as Proposition~1 predicts since those terms act only inside a debt-equal cohort. Second, the four variants are statistically indistinguishable in final accuracy, with the standard deviations in Table~\ref{tab:ablation4} overlapping in every pair. The only measurable effect of the two information terms is on convergence speed, where the complete score reaches $70\%$ absolute accuracy in $69.8$ rounds against $71.0$ for the debt term alone, a margin small relative to its spread across seeds. The fairness guarantee is therefore the primary result and the information terms refine the convergence path rather than the final accuracy. Repeating the comparison across $K \in \{1, 3, 5, 10\}$ leaves the spread between variants between $0.46$ and $1.58$ percentage points at every $K$ with no variant leading consistently, so the conclusion is not specific to the headline configuration.}

\subsection{Comparison with Ported Selectors}
\label{ssec:exp-baselines}

\rev{The selector is next compared against four published methods ported into the FD pipeline as described in Section~\ref{ssec:rw-fl-selection}, together with uniform random selection and the debt-only variant. Table~\ref{tab:baselines} reports the results at the headline configuration over five seeds.}

\begin{table}[t]
\centering
\caption{\rev{Selector comparison. FMNIST, $N=30$, $K=5$, $\alpha=0.5$, five seeds, mean $\pm$ standard deviation.}}
\label{tab:baselines}
\renewcommand{\arraystretch}{1.15}
\footnotesize
\setlength{\tabcolsep}{4pt}
...
\rev{The participation Gini coefficient is $1.33\%$ in every cell of the grid without exception, which confirms the prediction that the fairness guarantee is independent of the particular admissible values chosen. Final accuracy ranges from $70.94\%$ to $72.27\%$, so the entire grid spans $1.33$ percentage points, which is comparable to the seed-to-seed spread at a single setting. The configuration used throughout this paper, $(\alpha_u, \alpha_d) = (0.3, 0.1)$, reaches $71.99\%$ and sits within half a percentage point of the best cell. The selector therefore does not require the two coefficients to be tuned per configuration.}

\subsection{Data Heterogeneity}
\label{ssec:exp-heterogeneity}

\rev{The effect of the partition severity is examined by sweeping the Dirichlet concentration over $\alpha \in \{0.05, 0.1, 0.3, 0.5, 1.0, 5.0\}$ at $N=30$, $K=5$. Fig.~\ref{fig:dirichlet} shows the outcome.}

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figures/fig_r4_dirichlet_severity}
\caption{\rev{Effect of the Dirichlet concentration on FMNIST at $N=30$, $K=5$. Shaded bands are one standard deviation over seeds.}}
\label{fig:dirichlet}
\end{figure*}

\rev{Severe heterogeneity lowers accuracy for every selector alike. At $\alpha = 0.05$ the proposed selector reaches $26.74 \pm 4.19$ and uniform random reaches $27.07 \pm 4.24$, while at $\alpha = 5.0$ the two reach $84.67 \pm 0.11$ and $84.60 \pm 0.18$ respectively. Across the whole range the accuracy difference between the two remains inside one standard deviation. The participation Gini coefficient, in contrast, is $1.33\%$ for the proposed selector and $12.49\%$ for uniform random at every value of $\alpha$ tested. The fairness guarantee is therefore invariant to the degree of heterogeneity, as Proposition~1 predicts, since the debt term reads participation counts and not the data distribution. The two submodular baselines degrade on both axes as $\alpha$ falls, because their per-round diversity objective becomes less informative when each client holds few classes. At the extreme setting $\alpha = 0.01$ every selector collapses towards chance. Uniform random reaches $13.47 \pm 1.33$, the proposed selector $13.11 \pm 1.20$, and the two submodular baselines $12.95 \pm 0.96$. With most clients holding a single class the task is close to unlearnable under this budget, and no selection rule recovers it. The fairness separation nonetheless persists unchanged, at $1.33\%$ for the proposed selector against $12.16\%$ for uniform random and $53.37\%$ for the submodular baselines. As an upper anchor, an independent and identically distributed partition gives $85.68 \pm 0.07$ against $85.61 \pm 0.09$ for uniform random, with the same $1.33\%$ against $12.49\%$ fairness separation, so the gap persists even when the partition carries no heterogeneity at all.}

\subsection{Sparsity Sweep}
\label{ssec:exp-sparsity}

\rev{The behaviour under partial participation is characterized by sweeping $K \in \{1, 3, 5, 10\}$ at $N = 30$, with three seeds per point. Fig.~\ref{fig:ksweep} shows the result.}

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figures/fig_r3_k_sweep}
\caption{\rev{Sparsity sweep on FMNIST at $N=30$ over three seeds. Shaded bands are one standard deviation.}}
\label{fig:ksweep}
\end{figure*}
...
\rev{The sensitivity to the wireless link is characterized at $N = 30$, $K = 5$ by sweeping the downlink SNR over $\{-30, -25, -20, -15, -10\}$~dB with uplink SNR fixed at $-8$~dB, using three seeds per point. Fig.~\ref{fig:channel} shows the result.}

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_r7_channel_sweep}
\caption{\rev{Downlink SNR sweep on FMNIST at $N=30$, $K=5$ over three seeds. Shaded bands are one standard deviation.}}
\label{fig:channel}
\end{figure}
...
\rev{Federated distillation requires a shared public dataset, so the sensitivity of the method to that choice is examined along three axes, namely the identity of the public set, its size, and the level of label noise injected into it.}

\rev{Identity has the largest effect. Replacing the held-out public set with MNIST lowers the accuracy of the proposed selector from $71.75 \pm 0.81$ to $67.10 \pm 1.05$, which reflects the mismatch between the public inputs on which the consensus logits are formed and the private inputs on which the clients train. That cell also removes any overlap with the evaluation data, since MNIST is a separate corpus while the default public set comes from the held-out split of the private dataset. The ordering of the selectors survives it, with the debt-only variant at $67.14 \pm 0.95$ and uniform random at $66.72 \pm 0.89$. The accuracy level depends on the public set but the comparison between selectors does not.}
...
\rev{Label noise on the public set has no effect, and the reason is structural rather than empirical. The aggregation rule in (\ref{eq:agg}) consumes public-set logits and never reads public-set labels, so noise applied to those labels cannot enter the pipeline. The measured values at noise levels $0.1$ and $0.3$ match the noise-free baseline of $71.75 \pm 0.81$ exactly.}

\rev{Across all of these cells the proposed selector degrades in step with uniform random rather than more gracefully, the gap staying between $0.03$ and $0.38$ percentage points, so the public-set choice affects the pipeline as a whole rather than the selection rule. The participation Gini coefficient remains $1.33\%$ throughout.}
...
\rev{The ablation of Section~\ref{ssec:exp-ablation} explains why. The histogram enters the score only through the under-prediction bonus and the class-coverage penalty, and those two terms were already shown not to change final accuracy, so perturbing their input cannot change it either. The participation-debt term that carries the fairness guarantee never reads the histogram at all. The privacy-related claims of this paper therefore need no narrowing, because the disclosure can be removed at no measured cost.}
...
\rev{Under dropout the proposed selector holds both advantages together. At $p_{\text{drop}} = 0.1$ it reaches $71.16 \pm 0.69$ against $70.95 \pm 1.02$ for uniform random, at $p_{\text{drop}} = 0.2$ it reaches $70.41 \pm 1.00$ against $70.09 \pm 1.04$, and at $p_{\text{drop}} = 0.3$ it reaches $69.20 \pm 1.03$ against $69.16 \pm 1.48$. Accuracy falls for both as dropout rises, since fewer clients contribute to each round's consensus logits. The participation Gini coefficient of the proposed selector moves from $0.80\%$ to $1.88\%$ across that range while uniform random moves from $13.42\%$ to $15.45\%$. Dropout is the only perturbation examined here that moves the coefficient at all, because it is the only one that changes which clients actually participated. The debt term then rotates against the realised counts rather than the intended ones, which is the intended behaviour.}

\rev{Under bounded staleness the guarantee is untouched. At $W = 1$ the proposed selector reaches $71.71 \pm 0.61$ against $71.38 \pm 1.20$, and at $W = 2$ it reaches $71.54 \pm 0.60$ against $71.05 \pm 1.18$. The participation Gini coefficient is $1.33\%$ at both windows, identical to the synchronous value, because a late arrival changes when a contribution is aggregated but not whether the client was selected.}

\subsection{Channel and Energy Awareness}
\label{ssec:exp-channel-aware}

\rev{The motivation of this paper rests on an mMIMO substrate, yet the score in (\ref{eq:score}) reads no channel or energy quantity. A fourth term $\alpha_c q_i$ was therefore added, reading the per-client channel quality and energy estimate, and evaluated under an explicit per-round energy budget. The outcome is negative and is reported in full.}
...
\rev{The mechanism is the energy budget rather than the extra score term. Instrumenting the selection loop shows the budget acting as a hard feasibility filter, removing any client whose energy would exceed the remaining allowance regardless of how far behind it is. Under the plain selector every round admits the full cohort of $K = 5$. Under the channel-aware variant $89$ of $100$ rounds admit fewer, the mean cohort falls to $3.56$, the counts span $15$, and two of the thirty clients are never selected.}

\rev{This is a real boundary of the guarantee rather than an implementation defect. Proposition~1 assumes every client remains selectable so the debt ordering can be honoured, and a filter that removes clients on a criterion uncorrelated with debt breaks that assumption. Starving rounds of clients also thins each consensus logit, which accounts for the accuracy deficit. The proposed selector is therefore a fairness and data-coverage selector running on top of an mMIMO substrate rather than one that exploits channel state.}
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_r6_scale_fairness}
\caption{\rev{Participation fairness across client-pool and cohort sizes on FMNIST over five seeds. Each configuration carries one bar per selector. The filled circle is the cycle-aligned Gini coefficient at $R = 100$ and the open diamond is the rolling-window Gini coefficient measured over a window offset from the participation cycle, so the length of a bar shows how much the fairness reading depends on where the window is placed. Starred labels mark configurations in which $K$ does not divide $N$. The cycle-aligned coefficient of the proposed selector is zero exactly when $N$ divides $KR$, which is why $50/3$ reaches zero despite the star while $53/7$ and $47/6$ do not. Every value matches (\ref{eq:gini_exact}) to three decimal places.}}
\label{fig:scale}
\end{figure}

\rev{Three results follow. First, the guarantee strengthens rather than weakens as the pool grows. At $N = 200$ with $K = 10$ the proposed selector reaches a cycle-aligned Gini coefficient of $0.00\%$ against $24.16\%$ for uniform random, and its accuracy is $51.27\%$ against $48.84\%$. Second, the non-divisible configurations behave as (\ref{eq:gini_exact}) predicts, giving $1.40\%$ at $N = 47$ with $K = 6$ against $15.70\%$ for uniform random, and $1.25\%$ at $N = 53$ with $K = 7$ against $13.81\%$. Third, the advantage survives a measurement window that does not align with a cycle, since the rolling-window coefficient of the proposed selector stays between $4.52\%$ and $18.58\%$ across all configurations while uniform random stays between $43.85\%$ and $52.22\%$. The guarantee can also be read at any horizon, because every run records the full per-round history. At the headline configuration the coefficient falls from $3.33\%$ at $R = 25$ to $1.33\%$ at $R = 100$, matching (\ref{eq:gini_exact}) at every horizon and staying below the bound $N/(4KR)$, while uniform random falls only from $23.93\%$ to $12.49\%$.}

\rev{The one axis on which the proposed selector does not lead is accuracy at high participation ratios, and the campaign is large enough to make that statement quantitative. Fifteen configurations pair it against uniform random on matched seeds under otherwise identical settings, covering $N$ from $30$ to $200$ and ratios $K/N$ from $0.033$ to $0.333$. Fig.~\ref{fig:ratio} collects them. The mean accuracy gap falls monotonically as the ratio grows, at a Spearman rank correlation of $-0.815$ against $K/N$ with $p = 0.0002$, reaching $+2.43$ percentage points at $N = 200$ with $K = 10$ and $-1.42$ at $N = 200$ with $K = 40$. Twelve of the fifteen gaps are positive.}

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_r8_ratio_gap}
\caption{\rev{Final accuracy relative to uniform random selection on FMNIST at $\alpha = 0.5$, for every matched configuration in the campaign, ordered by the participation ratio. Each row pairs the complete score with the debt-only variant on the same seeds, and the grey connector is the distance between them. Whiskers are one standard deviation of the paired difference over seeds, with five seeds per configuration except at $30/1$, $30/3$ and $30/10$, where three are used. The advantage of the complete score shrinks as the ratio grows and turns negative in the three configurations with a cohort of at least twenty clients, while the debt-only variant stays on the positive side of zero throughout.}}
\label{fig:ratio}
\end{figure}
...
\rev{Balanced rotation therefore costs no accuracy at any ratio tested and helps most where participation is sparse, while the two information terms are what turn the gap negative once the cohort is large enough to cover the label space on its own. This agrees with the ablation of Section~\ref{ssec:exp-ablation}. A deployment at a high participation ratio should prefer the debt-only variant, and the accuracy claim of this paper is confined to the sparse regime.}

\rev{The per-round record shows the rotation directly. At $(N = 30, K = 10, R = 100)$ over three seeds, uniform random spreads the cumulative counts from $23$ to $42$ with a standard deviation of $4.53$, while every client under the proposed selector sits at $33$ or $34$ with a standard deviation of $0.47$, a reduction of about $9.6\times$ and the value (\ref{eq:gini_exact}) implies. Each window of $\lceil N/K \rceil = 3$ consecutive rounds aligned to the start of training partitions the pool exactly, in $33$ of $33$ windows on all three seeds and for both variants, which uniform random achieves in none. The two differ in how they fill the cycle rather than whether they complete it. The debt-only variant repeats exactly three cohorts, the round-robin it reduces to, while the complete score draws on between $35$ and $54$ distinct cohorts and still partitions every aligned window. This is the behaviour predicted at the end of Section~\ref{ssec:participation-balance}.}

\subsection{Results on Further Datasets}
\label{ssec:exp-cross-dataset}

\rev{Two further datasets are run under the same multi-seed protocol to test whether the conclusions depend on Fashion-MNIST. The first varies the image corpus and the second leaves images altogether.}
...
\rev{To test a domain outside images the selector is run on the Free Spoken Digit Dataset, a ten-class spoken-digit corpus represented as log-spectrograms and paired with the \texttt{AudioCNN} model. Two settings differ from the image studies, both forced by the size of the corpus. It provides roughly ninety training samples per client, about twenty times less than Fashion-MNIST, so the local epochs are raised from one to three, and its held-out split holds only three hundred samples, so the public set is cut to one hundred and fifty to keep it from coinciding with the evaluation split. Over five seeds the proposed selector reaches $46.16 \pm 1.69$, the debt-only variant $45.54 \pm 1.17$, uniform random $44.99 \pm 1.59$, SubTrunc $40.66 \pm 1.48$ and DivFL $40.53 \pm 1.49$, with the participation Gini coefficient at $1.33\%$ against $12.49\%$ for uniform random and about $45\%$ for the two submodular methods. The proposed selector leads uniform random on every one of the five seeds, by between $0.51$ and $1.70$ percentage points, and both submodular methods on every seed by at least four. Absolute accuracy is far below the image results because of the small per-client sample count, so the ordering rather than the level is informative here. That ordering reproduces the image result, including the tie between the complete score and the debt-only variant, the latter trailing by $0.61$ percentage points at a two-sided paired $p$ of $0.188$.}

\rev{Across the three datasets the fairness separation is identical, because it depends only on the participation arithmetic, and the accuracy ordering is preserved throughout. What changes is the absolute level, which reflects the difficulty of the task rather than the selection rule.}

\subsection{Invariance of the Fairness Guarantee}
\label{ssec:exp-invariance}

\rev{Taken together the experiments above support a stronger statement than that the proposed selector is fair. The campaign spans seventeen experiment families, so it is large enough to test (\ref{eq:gini_exact}) as a prediction rather than to report fairness one setting at a time.}

\rev{The test is direct. For every run, $N$, $K$ and $R$ are known before the run starts, so (\ref{eq:gini_exact}) fixes the participation Gini coefficient in advance. Across the campaign it is correct in $457$ of $457$ cases, covering $275$ runs of the proposed selector and $182$ of the debt-only variant, with a largest deviation below $10^{-4}$ percentage points. Held out from that count are the eighteen dropout runs, where a selected client may fail to return and the realised participation is therefore not the intended participation. The measured value is also independent of the seed, with a standard deviation of exactly zero over seeds at every configuration, which is what a deterministic rule should give.}

\rev{The law holds across every condition reported in Sections~\ref{ssec:exp-coefgrid} to~\ref{ssec:exp-cross-dataset}, that is across all three datasets, all seven heterogeneity levels, the whole SNR sweep, every privacy mechanism, both staleness windows, every public-set variation and all thirty cells of the coefficient grid. The value $1.33\%$ is returned in $202$ runs spanning fifteen of the seventeen families, because those runs share the headline arithmetic, and every remaining run returns the value the law specifies for its own triple.}

\rev{This follows from the structure of the score. The right-hand side of (\ref{eq:gini_exact}) contains only $N$, $K$ and $R$, because the participation-debt term reads participation counts alone, so no perturbation that leaves those three fixed can move the coefficient whatever it does to the data, the channel, the public set or the privacy mechanism. Fairness can therefore be predicted from the deployment parameters instead of being re-validated for each deployment condition, which is what a penalty-based or stochastic fairness objective cannot offer. The one condition under which it degrades, a hard resource constraint that renders clients unselectable, is characterised in Section~\ref{ssec:exp-channel-aware}. Dropout is the milder form of the same effect, and the coefficient then varies from run to run between $0.42\%$ and $2.12\%$.}
...
This paper has proposed SCOPE-FD, a deterministic, fairness-first client selector for federated distillation over massive MIMO networks under partial participation, built on the FedTSKD substrate of \cite{mu2024fedtskd}. The selector acts on the over-round composition of the client subset through a debt-dominated three-term score with fixed non-negative coefficients.

\rev{The central result is that the fairness of the selector is predictable rather than merely good. The cumulative participation Gini coefficient is $m(N-m)/(NKR)$ with $m = KR \bmod N$, so it follows from the pool size, the cohort size and the horizon alone, and that value was returned in all $457$ selector runs of the study free of client dropout. At the headline configuration it falls from $12.49\%$ under uniform random selection to $1.33\%$ while accuracy is matched at $71.21 \pm 0.99$ against $70.99 \pm 1.38$. Against published selectors ported into FD the proposed method improves on DivFL by $1.33$ and on SubTrunc by $1.81$ percentage points, while a utility-based FL selector transferred unchanged reaches only $21.14 \pm 0.60$.}

\rev{That guarantee also carries the convergence statement. Composed with the per-client bound of \cite{mu2024fedtskd} it gives a rate of $\mathcal{O}(N/(KR))$ in communication rounds that holds for every client at every finite horizon, adds no assumption to those of \cite{mu2024fedtskd}, and returns their full-participation rate once the cohort is the whole pool.}

\rev{Three limits are stated plainly. The accuracy advantage belongs to the sparse regime, declining with $K/N$ at a rank correlation of $-0.815$ and turning negative once the cohort reaches twenty clients, where the debt-only variant should be preferred. Label-histogram disclosure is removable at no measured cost. A hard energy budget, in contrast, breaks the guarantee, because filtering clients on a criterion uncorrelated with participation debt violates the assumption of Proposition~1 that every client remains selectable.}

\new{Four extensions are left open. The first is to admit channel and energy signals into the score while preserving the participation guarantee, which the energy-budget result shows is not straightforward. \rev{The second is a convergence rate that separates the deterministic rotation from uniform random selection in order rather than only in the strength of the statement, which would need the without-replacement structure to be exploited inside the bound instead of only in the participation counts.} The third is a ratio-dependent weighting of the two information terms, since Section~\ref{ssec:exp-scale} locates their cost at high participation ratios. \rev{The fourth is a wider study of non-image domains, on corpora large enough for absolute accuracy to be informative, which the spoken-digit corpus used here is not.}}
\balance
% \IEEEtriggeratref{19}
