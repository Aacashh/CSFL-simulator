...
\rev{Proposition~1 assumes every client remains selectable, and a filter acting on a criterion uncorrelated with debt breaks that assumption. Starving rounds of clients also thins each consensus logit, which accounts for the accuracy deficit. The proposed selector is therefore a fairness and data-coverage selector running on top of an mMIMO substrate rather than one that exploits channel state.}
...
\new{Four extensions are left open. The first is to admit channel and energy signals into the score while preserving the participation guarantee, which the energy-budget result shows is not straightforward. \rev{The second is a rate that exploits the without-replacement structure inside the bound itself rather than only through the participation counts.} The third is a ratio-dependent weighting of the two information terms, since Section~\ref{ssec:exp-scale} locates their cost at high participation ratios. \rev{The fourth is a wider study of non-image domains, on corpora large enough for absolute accuracy to be informative, which the spoken-digit corpus used here is not.}}


\rev{What the paper establishes is therefore a selector whose fairness is a design parameter rather than an outcome: given the pool, the cohort and the horizon, the participation imbalance of a deployment is known before it is switched on.}
