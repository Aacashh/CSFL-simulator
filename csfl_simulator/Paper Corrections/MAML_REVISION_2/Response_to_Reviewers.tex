...
\ManuscriptChange{The System Model explains $\lambda$. The Evaluation reports the Fashion-MNIST sweep in Fig.~5 and the CIFAR-10 sweep in Table~V.}
...
\ManuscriptChange{The Evaluation now includes Fashion-MNIST, CIFAR-10, and CIFAR-100, together with the selector time-complexity argument and the scaling measurement in Fig.~4.}
...
\ManuscriptChange{Table~II reports accuracy, weighted precision, recall, F1, TFLOPs, modelled energy and carbon, Jain fairness, and coverage, each with a standard deviation, and its caption states the seed count per block. Table~III reports the paired tests against FedAvg.}
...
\ManuscriptChange{Table~II reports Jain fairness and coverage, and the Evaluation states that MAML-Select is not the fairest method and reports both metrics recomputed without the opening round-robin rounds.}
...
\ManuscriptChange{The State Space paragraph explains the role of each feature, Table~VI reports the feature ablation, and Table~V reports the inner-step count.}

\ReviewerComment{Comment 9}{Fig. 2 has small font sizes and unclear axis scaling.}
\ReviewerReply{Reply 9}{We regenerated every figure with larger fonts and cleaner axes. The manuscript now carries four evidence figures, for convergence, the efficiency and accuracy trade-off, selector scaling, and $\lambda$ sensitivity. The feature ablation is reported as a table instead, because the plot showed the same seven numbers. We also rebuilt the scaling figure, whose panel labels overlapped and whose measurement range did not match the text.}
...
\ReviewerReply{Reply 11}{We added both methods to the benchmark table and the related-work discussion. On CIFAR-100, FedGCS reaches $58.66 \pm 0.11$ against $58.15 \pm 0.51$ for MAML-Select at comparable cost, and CriticalFL reaches $45.37 \pm 6.13$ while using about $64\%$ more compute than MAML-Select. We report the FedGCS result plainly because it is the closest competitor on that benchmark.}
\ManuscriptChange{Related Work, Table~I, Table~II, and the evidence figures now include FedGCS and CriticalFL.}
...
\ManuscriptChange{The manuscript carries the convergence and trade-off evidence, the $\lambda$-sensitivity figure, the scaling measurement, the paired significance tests, and the selector-diagnostics table. It is self-contained and no separate supplement is submitted.}
...
\ReviewerReply{Reply 6}{The original figure has been replaced and the presentation has been clarified. The manuscript now presents convergence, the efficiency and accuracy trade-off, selector scaling, and $\lambda$ sensitivity with clearer axes and captions, and reports the feature ablation as a table so that the same numbers are not shown twice.}
\ManuscriptChange{Every figure and caption was regenerated and rewritten.}
