...
\revision{Whether the adapted policy makes a better decision is a separate question. We therefore evaluated the query objective twice per round, once at the stored initialization and once after the inner step, and took the difference as the adaptation gain. Adaptation helps in close to three rounds out of five rather than in every round, and its mean is positive on all three datasets. To measure how far the objective itself travels, we evaluated the previous and the current round objective at one parameter and summed the absolute per-round increments. This path variation $V_T$ is the standard way to quantify a moving objective.}

\revision{Table~\ref{tab:drift} reports both. The mean gain follows the same order as $V_T$ across the three datasets, so adaptation contributes most where the objective moves most. That is the behaviour a meta-learned initialization is meant to produce, and it is the empirical content of the claim that the formulation suits non-stationary selection. Three datasets support an ordering and not a significance result, and we report it as such.}
...
\revision{To verify~\eqref{eq:selector_complexity}, we measured selection overhead for $50$ rounds while increasing $N=\{20,40,80,100\}$ at fixed $K/N=0.1$. Measured server-side overhead stays near $11$ to $13$ ms per round and the normalized analytical work follows $NP+N\log K+2KP$. With $K$ and $P$ fixed the dominant term is linear in $N$, and the cost stays small next to local client training.}
