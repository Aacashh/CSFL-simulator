...
## What was done, 14 August 2026

All four documents build clean. Manuscript **6 pages**, response letter 8,
supplementary 4, zero errors, zero undefined references, zero overfull boxes,
zero BibTeX warnings, and `MAML_Select_Revision2.zip` builds from a bare
extract. Advait set the hard cap at **8 pages**.

### Manuscript

- Table II CIFAR-100 block regenerated by `emit_rows.py`. TiFL and FedCor now
  carry a real second seed, CriticalFL a third. The caption states the seed
  count per block and notes that recall equals accuracy under this protocol.
- New subsection **Selector Convergence and Non-Stationarity** with **Table III**.
- Fairness paragraph gained the cold-start recomputation.
- Lambda paragraph gained the CIFAR-10 contrast, the ablation gained the
  inner-step result, and battery/loss was corrected to battery/latency.
- Style: zero colons, semicolons or dashes remain in prose. What is left is
  set-builder notation and the argument separator inside the loss.

### The selector-scaling defect, found by looking at the rendered page

Three things disagreed and none of them would have shown up in the source.

1. The text said the probe ran **50 rounds**. Both campaigns record **10**.
2. The text quoted **11 to 13 ms** over N up to 100, which is the **June 5**
   campaign at `runs/maml_select/scaling/`. The figure plotted the **June 10**
   campaign at `Paper Corrections/scaling/`, N to 1000, at **20 to 26 ms**.
3. The figure's panel titles and axis labels overlapped badly.

Resolved by standardising on the June 10 campaign, which is newer, reaches
N = 1000 and has tighter variance, and regenerating the figure with
`maml_data_verification/make_scaling_figure.py` at the printed width so nothing
is rescaled. The supplementary table was switched to the same campaign and its
units corrected from seconds to milliseconds.

**An honest reading came out of this.** Measured overhead is flat while the
operation count grows fiftyfold, so at these pool sizes the cost is dominated
by fixed per-call overhead, not by the linear scoring term. The text now says
that rather than implying the measurement confirms linear growth.
...
|---|---|
| round-1 baseline | 6 |
| new evidence added | 7 |
| feature-ablation figure dropped, five passages tightened | 7 |
| CodeCarbon entry shortened from 29 names | **6** |
| Table III added | 7 |
| style pass, which also shortened sentences | 7 |
| six author lists abbreviated | **6** |
| scaling paragraph corrected and lengthened | 7 |
| four passages tightened | 7 |
| eleven venue strings abbreviated | **6** |

The lesson from SCOPE-FD repeated exactly. **The layout is float-constrained.**
Prose cuts stopped propagating twice, and both times the bibliography was what
actually moved the page count.

### Checks that should be re-run after any edit

```bash
python maml_data_verification/audit.py    # 20 of 20 must pass
```

It reads the **rendered PDF**, not the source, so it checks what a reviewer
sees. It also fails if a float loses its in-text reference, which happened once
when the scaling paragraph was rewritten and `fig:scaling_overhead` was left
defined but never cited.

### The heredoc gotcha bit again

Writing `Fig.~\ref{...}` inside a `<<'PY'` heredoc produced `Fig.~<CR>ef{...}`
