...
\revision{In Federated Learning (FL), choosing which clients update the shared model each round is an NP-hard scheduling problem under resource constraints. Clients differ in data utility, training speed, battery capacity and bandwidth, and only a small cohort can communicate per round. Conventional strategies favour high-utility or fast clients, so they repeatedly pick a narrow group and reduce participation coverage. FedCS and TiFL use system state to limit straggler effects, while Oort, FedCor, CriticalFL, and FedGCS score clients by utility, loss correlation, critical-period timing, or learned combinations. All rely on a fixed scoring rule, or update it slowly, even though a client's usefulness changes as the model evolves. \textbf{Model-Agnostic Meta-Learning Select (MAML-Select)} removes that fixed-rule assumption and frames each round as a fast adaptation task. A lightweight policy network is adapted from recent loss-reduction and latency feedback, used to rank clients, and then meta-updated once the new feedback arrives. \revtwo{We derive the per-client ranking score from the cohort-level objective and prove that the ranking rule exactly minimizes the resulting surrogate. We also give a tracking guarantee that holds when client utility drifts, rather than assuming it is fixed.} Across Fashion-MNIST, CIFAR-10, and CIFAR-100 with $100$ clients, $K=10$, a Dirichlet split at $\alpha=0.5$ and three seeds, MAML-Select gives competitive accuracy at markedly lower cumulative Tera Floating-Point Operations (TFLOPs) and modelled energy, while reaching full participation coverage.}
\end{abstract}
\renewcommand\IEEEkeywordsname{Impact Statement}
\begin{IEEEkeywords}
\revision{FL underpins privacy-preserving AI in mobile health, intelligent transportation, industrial sensing, and smart-city systems, yet these deployments face tight compute, energy and latency budgets. MAML-Select offers an adaptive selection mechanism that cuts unnecessary client-training cost while preserving broad participation coverage. Because it re-ranks clients from recent training feedback, it suits FL systems that must respond to changing device conditions rather than follow a fixed rule. The reported energy values are modelled estimates from the declared simulator and device-tier setup, which gives a transparent basis for comparing selection policies. The broader impact is a more resource-aware path for edge AI, including the industrial Internet of Things (IoT).}
...
\IEEEPARstart{F}{ederated} Learning (FL) trains a shared model across many devices without moving raw data to the server~\cite{mcmahan2017communication}. \revision{Clients train locally and share only updates, yet they differ in hardware, network conditions, and local data. That heterogeneity creates statistical drift under non-independent and identically distributed (non-IID) data and straggler delays under synchronous training}~\cite{karimireddy2020scaffold,mai2024study,tapwal2026stragglers,forootani2026asynchronous,borazjani2026redefining}. The problem of \textit{whom to select} each round remains open.

Client selection chooses a subset $\revision{\mathcal{S}_t} \subset \{1, \ldots, N\}$ each round. \revision{Client utility is non-stationary, because a useful client can become redundant later while an overlooked client can become important.} System-aware methods raise participation but bias selection against slow yet data-rich clients~\cite{nishio2019client,chai2020tifl}. Utility-aware methods prioritize information gain at the cost of higher latency or expensive server-side modelling~\cite{lai2021oort,tang2022fedcor,xu2024federated,yan2023criticalfl,ning2024fedgcs}. Learning-based selectors use histories, features, or reinforcement signals~\cite{wang2020optimizing,pan2024contextual,kazemi2026robust}. \revision{An efficient round-by-round mechanism for adapting the ranking policy to evolving utility remains underexplored.}

\revision{We propose MAML-Select, a client-selection framework built on a Model-Agnostic Meta-Learning (MAML)-style fast adaptation step~\cite{finn2017model} that updates the policy network from recent feedback before the next ranking decision. Unlike reinforcement-learning (RL) or contextual-bandit selectors, it adapts the scoring network itself by gradient descent. Unlike personalization of FL client models}~\cite{fallah2020personalized}, \revision{it adapts the selector rather than the client model, which helps when devices have limited compute, strict deadlines, or intermittent connectivity.} The major contributions are as follows.
\begin{enumerate}
\setlength{\itemsep}{0pt}
\setlength{\topsep}{1pt}
		\item We formulate FL client selection as a meta-learning task and introduce the MAML-Select algorithm with \revision{rapid gradient-based} adaptation.
		\item \revtwo{We derive the per-client score from the cohort objective, prove that Top-$K$ exactly minimizes the resulting surrogate, and give a convergence guarantee that tolerates a drifting objective instead of assuming a fixed one.}
        \item \revision{Experiments on Fashion-MNIST, CIFAR-10, and CIFAR-100 at $\alpha = 0.5$ over three seeds show accuracy competitive with FedAvg and FedGCS at lower computational cost, and higher accuracy than FedCS, Oort, TiFL, and FedCor. MAML-Select \textit{lowers cumulative training TFLOPs} by up to $21.5\%$ and modelled energy by up to $24.7\%$ relative to FedAvg, with paired-test support.}
\end{enumerate}
...
\begin{table*}[!t]
    ...
    \begin{tabularx}{\textwidth}{lXXX}
        ...
        Per-FedAvg~\cite{fallah2020personalized} & Personalize the client model to local data & MAML adaptation of the client model, not the selector & No client selection; per-client model fine-tuning \\
...
\revision{At round $t$, let $\mathcal{A}_t \subseteq \mathcal{N}$ be the available clients and $\mathcal{S}_t \subseteq \mathcal{A}_t$ the selected cohort. The server holds the global model and the selection policy, while clients keep data local and return only model updates, loss summaries, and resource-state feedback. Selection is therefore a server-side decision taken before each broadcast.}

\revision{Client $i$ has per-round latency $T_{i,t}=T_{i,t}^{comp}+T_{i,t}^{comm}$. The computation time is $T_{i,t}^{comp} = E\, n_i\, C_{model}/f_{i,t}$, where $E\, n_i\, C_{model}$ is the local-training work over $E$ epochs and $n_i$ samples, $C_{model}$ is the per-sample forward-plus-backward cost in FLOPs, and $f_{i,t}$ is the device processing rate set by its hardware tier. The communication time is $T_{i,t}^{comm} = M/B_{i,t} + \delta_{i,t}$, for an update of size $M$, uplink bandwidth $B_{i,t}$, and fixed delay $\delta_{i,t}$~\cite{nishio2019client}. In synchronous FL the slowest selected client sets the round latency,}
\begin{equation}
	\color{black}
...
\revision{Building on~\eqref{eq:bi_objective}, MAML-Select treats client selection as a meta-learning problem, because client utility changes as training progresses. The algorithmic details of the selection policy are given below.}

\subsubsection{State Space}
\revision{The server builds a state vector $\bm{s}_{i,t} \in \mathbb{R}^6$ for each client $i$ at round $t$. It holds the local training loss $\hat{L}_{i,t}$, the gradient norm $\|\nabla F_i\|_2$, the estimated latency $\hat{T}_{i,t}$, the battery level $b_{i,t}$, the selection frequency $q_{i,t}$, and the staleness $\tau_{i,t}$ measured in rounds since the client was last selected}~\cite{pan2024contextual,xu2024federated}. The three pairs capture statistical utility, system feasibility, and participation balance in turn. Each round $t$ is a task $\mathcal{T}_t$ in which clients are ranked so that the top-$K$ cohort minimizes the per-round utility.
...
MAML-Select trains a meta-policy network $h_\phi(\bm{s}_{i,t})$, parameterized by $\phi$, which is adapted to the current round using recent feedback. \revision{The policy is a $6$-$64$-$64$-$1$ multilayer perceptron (MLP) with ReLU activations and 4,673 trainable parameters.} \revtwo{Cost feedback for a cohort becomes observable only after that cohort has finished training, so the two meta-learning sets are drawn from two consecutive completed rounds. Writing $\mathcal{S}_t$ for the cohort of round $t$, the support and query sets used at round $t$ are}
\begin{equation}
	\revtwoeq \mathcal{D}_{sup}(t) = \bigl\{(\bm{s}_{i,t-2},c_{i,t-2})\bigr\}_{i\in\mathcal{S}_{t-2}},\quad
	\mathcal{D}_{query}(t) = \bigl\{(\bm{s}_{i,t-1},c_{i,t-1})\bigr\}_{i\in\mathcal{S}_{t-1}} ,
	\label{eq:sets}
\end{equation}
\revtwo{and therefore $\mathcal{D}_{sup}(t)=\mathcal{D}_{query}(t-1)$, so the support set is one round staler than the query set. This lag is what makes the procedure a meta-learning problem rather than online regression, because the inner step adapts on an older round while the meta-gradient is measured on a newer one. The outer update therefore rewards initializations from which one adaptation step transfers forward in time, which is what a selector needs when utility drifts. The candidate set $\mathcal{D}_{cand}(t)=\{\bm{s}_{i,t}\mid i\in\mathcal{A}_t\}$ carries no labels and is only scored.}

\subsubsection{\revtwo{From the Cohort Objective to a Per-Client Score}}
...
\end{proof}
A single support-set step therefore reduces the round's ranking loss, which justifies adapting before the candidates are scored.
}
...
\textbf{Phase~3\revtwo{, client selection and FL execution}.}
...
\begin{equation}
	...
	\label{eq:selection}
\end{equation}
\revtwo{Two rules qualify~\eqref{eq:selection} and are part of the method as run. During the first $\lceil N/K\rceil$ rounds the policy has too little feedback to rank anything, so a cold start takes the cohort in a fixed seed-shuffled coverage order that visits every client once and seeds the feedback buffer. Thereafter an exploration slot gives one of the $K$ places to the stalest and least-used available client, ordered by $(-\tau_{i,t},\,q_{i,t})$, and the remaining $K-1$ places to the Top-$(K-1)$ of the learned ranking. The slot is deterministic rather than random, is set to $\varepsilon=1$ throughout, and bounds how long any client can stay unobserved. Sec.~\ref{sec:fairness_recheck} verifies that the reported coverage does not depend on the cold start.}
...
\]
denote the \revtwo{\emph{look-ahead displacement}}, the gap between the direction actually applied, $\nabla q_t(\phi'_t)$, evaluated at the adapted weights $\phi'_t$, and the query gradient $\nabla q_t(\phi_t)$ at the un-adapted weights. If the outer step size obeys $0<\eta\le 1/L_q$, the meta-update~\eqref{eq:outer_loop} satisfies
\[
...
\revtwo{If the environment settles, so that $V_T=o(T)$, the policy reaches an $\mathcal{O}(\beta^2)$-approximate stationary point at rate $\mathcal{O}(1/(\eta T))$, and a fixed objective is the special case $V_T\le 0$. If utility drifts at a constant rate the drift term does not vanish and the guarantee degrades to tracking, meaning the policy stays within a drift-proportional neighbourhood of stationarity. We therefore make no claim of convergence to a stationary point of a fixed objective. The analysis also assumes an outer step $\phi_{t+1}=\phi_t-\eta\nabla q_t(\phi'_t)$, whereas the implementation passes that direction through Adam, so it characterizes the applied direction but not Adam's per-coordinate rescaling.}
\end{remark}
\color{black}
\revtwo{Fig.~\ref{fig:convergence}(b) is consistent with this reading, since the meta-update magnitude on CIFAR-100 shrinks and then settles at a small non-zero level rather than decaying to zero.}
}
...
\begin{algorithm}[!t]
	...
	\begin{algorithmic}[1]
		...
		\State \revision{$\text{score}_i \leftarrow h_{\phi'_t}(\bm{s}_{i,t})$ for all $i \in \mathcal{A}_t$} \Comment{Phase 3}
			...
			\Else
			\State \revtwo{$\mathcal{E}_t\leftarrow$ $\varepsilon$ clients of $\mathcal{A}_t$ largest in $(-\tau_{i,t},\,q_{i,t})$.} \Comment{\revtwo{exploration}}
...
\revision{MAML-Select differs from fixed-rule selectors because its policy is adapted before each decision, and from RL selectors because the update is an explicit gradient step on recent feedback.}
...
\end{equation}
\revision{To validate~\eqref{eq:selector_complexity} we measured the selection overhead over $10$ rounds for $N\in\{20,40,80,100,200,500,1000\}$ with the selection rate held at $10\%$. Fig.~\ref{fig:effscale}(b) shows it staying between about $20$ and $26$ ms per round as $N$ grows fifty-fold, while the operation count grows about linearly in Fig.~\ref{fig:effscale}(c), because at these sizes the small $\mathcal{O}(NP)$ scoring cost is dominated by fixed per-round bookkeeping. Since $P$ and $K$ are fixed in the protocol, MAML-Select scales linearly with $N$. The memory cost is $\mathcal{O}(P+N+K)$ for the policy parameters, client scores, and selected cohort. This results is in contrast with correlation-matrix or Gaussian-Process selection approaches where the covariance operations can grow beyond linearly, and in common FedCor-style implementations cubically, with $N$~\cite{tang2022fedcor}.}
...
\begin{equation}
	...
	\label{eq:tflops}
\end{equation}
\revision{where $C_{model}$ matches $T_{i,t}^{comp}$, so cost and latency stay consistent, and one ResNet18 round on CIFAR-10 is about $4.13$ TFLOPs. Modelled energy accumulates per-tier power over local-training time,}
\begin{equation}
\color{black}
...
\revision{Table~\ref{tab:main_summary} summarises all methods. MAML-Select matches FedAvg and FedGCS on Fashion-MNIST and CIFAR-100, and exceeds FedCS, Oort, TiFL, FedCor, and CriticalFL on CIFAR-100. On CIFAR-10 it gives up $3.8$~pp against FedAvg for substantially lower cost. It therefore offers a resource-aware operating point close to the strongest accuracy baselines, and its meta-update overhead of about $0.001$ GFLOPs per round is negligible next to client training.}

\revision{Paired $t$-tests over the three seeds support the resource reductions against FedAvg on every dataset, with TFLOPs $p$-values of $0.009$, $0.002$ and $0.012$ and energy $p$-values of $0.007$, $0.001$ and $0.003$. Against FedGCS the accuracies are indistinguishable on CIFAR-10 ($p=0.10$) and CIFAR-100 ($p=0.49$). Against FedAvg they are equivalent on Fashion-MNIST ($-0.1$~pp, $p=0.84$), while CIFAR-10 ($-3.8$~pp) and CIFAR-100 ($-1.9$~pp) trade a small accuracy loss for the best energy efficiency. Lowering $\lambda$ from $0.5$ to $0.1$ raises CIFAR-10 diagnostic accuracy by only about one point, so the residual gap reflects the harder ResNet18 task rather than the $\lambda$ setting, and we read it as an efficiency-oriented operating point.}

\begin{table*}[!t]
\centering
\color{black}
\caption{Benchmark summary across non-IID datasets. Accuracy, F1, and coverage are in percent, and energy is in Watt-hour (Wh). \revtwo{Precision and recall are omitted because recall coincides with accuracy under micro-averaging.}}
...
TiFL & 32.12$\pm$0.30 & 31.02$\pm$0.42 & 5986$\pm$28 & 3499$\pm$14 & 0.11$\pm$0.00 & 12$\pm$0 \\
FedCor & 34.18$\pm$0.24 & 32.93$\pm$0.20 & 6428$\pm$32 & 3668$\pm$18 & 0.12$\pm$0.00 & 14$\pm$0 \\
CriticalFL & 48.84$\pm$1.58 & 48.07$\pm$2.32 & 9469$\pm$392 & 5438$\pm$214 & 0.97$\pm$0.00 & 100$\pm$0 \\
...
\endgroup
\end{table*} 
\revision{Fig.~\ref{fig:convergence} summarises the CIFAR-100 behaviour. MAML-Select stays close to FedAvg and FedGCS on accuracy and loss while remaining in the low-energy cluster, and panel~(b) shows the selector meta-update settling.}
...
\revision{Fig.~\ref{fig:effscale}(a) shows the efficiency and accuracy trade-off. Relative to FedAvg over shared seeds, MAML-Select lowers cumulative TFLOPs by $12.8\%$, $21.5\%$ and $1.7\%$ and modelled energy by $16.4\%$, $24.7\%$ and $4.7\%$ on the three datasets, for accuracy changes of $-0.1$, $-3.8$ and $-1.5$ percentage points. The main empirical gain is therefore resource efficiency at competitive accuracy.}

\begin{figure}[!tb]
\centering
\includegraphics[width=\columnwidth]{fig_efficiency_scaling_boxed.pdf}
\caption{\revision{(a)~Efficiency and accuracy trade-off. The $x$-axis is cumulative training cost relative to FedAvg on a log scale, where $x<1$ is cheaper than FedAvg, the $y$-axis is final accuracy, and marker shape encodes the dataset. (b)~Measured per-round selection overhead, reported as a mean with $95\%$ confidence intervals over $10$ rounds. (c)~The analytical operation count $C(N,K)=NP+N\log K+2KP$ from~\eqref{eq:selector_complexity} with $P=4{,}673$, normalized to its value at $N=20$. Panels (b) and (c) vary the pool size $N$ with the selection rate held at $10\%$.}}
\label{fig:effscale}
\end{figure}

\revision{Participation fairness uses Jain's index on the per-client selection counts $\{p_i\}_{i=1}^{N}$, $J=\big(\sum_{i} p_i\big)^2 / \big(N\sum_{i} p_i^2\big)$, which lies in $[1/N,1]$ and equals $1$ for perfectly even selection. Participation coverage $\mathrm{Cov}=\tfrac{100}{N}\,|\{\,i:\sum_{t} a_{i,t}\ge 1\,\}|$ is the percentage of clients selected at least once. MAML-Select reaches $100\%$ coverage on every dataset, with a Jain index of $0.61$ to $0.78$.} \revtwo{The two answer different questions. Coverage is guaranteed by the exploration slot (Proposition~\ref{prop:coverage}) independently of the learned ranking, whereas the Jain index measures how evenly the remaining $K-\varepsilon$ places are spread. A latency-aware selector uses fast devices more often, and Sec.~\ref{sec:tier} shows this is what the lower index reflects.}

\subsection{\revtwo{Tier-Level Participation}}\label{sec:tier}
\revtwo{Coverage says how evenly clients are used but not which ones, so we also measured the share of selections per device tier on CIFAR-100 against a pool composition of $0.20/0.50/0.30$. FedCS collapses onto the fast tier, placing all $1.000$ of its selections in Tier~3, never selecting a Tier~1 device, and reaching only $10\%$ coverage. FedAvg tracks the pool at $0.196/0.495/0.309$ and FedGCS at $0.217/0.472/0.311$, both with full coverage. MAML-Select shifts toward the fast tier at $0.116/0.446/0.437$, yet keeps the slow tier represented and retains $100\%$ coverage. The distinction that matters is between preferring fast devices and excluding slow ones, and only the former is compatible with Proposition~\ref{prop:coverage}.}

\subsection{\revtwo{Coverage Without the Forced Cold Start}}\label{sec:fairness_recheck}
\revtwo{The cold start visits every client once in the first $10$ of $200$ rounds, so the coverage figures could be an artifact of that schedule rather than a property of the policy. Recomputing both metrics over the policy-driven rounds alone, across all $74$ MAML-Select runs and every $\lambda$, inner-step, width and feature-ablation setting, coverage is $100\%$ in every individual run. Jain's index falls only slightly, from $0.755$ to $0.737$, $0.411$ to $0.367$, and $0.802$ to $0.785$ on the three datasets. The following result shows why.}

\begin{proposition}[\revtwo{Coverage from the exploration slot alone}]\label{prop:coverage}
\revtwo{Suppose every client is available in every round and $\varepsilon\ge 1$. Then, after the cold start, every client is selected at least once in any window of $N$ consecutive rounds, independently of the policy $h_\phi$.}
\end{proposition}
\begin{proof}
\revtwo{Staleness obeys $\tau_{i,t+1}=0$ if $i\in\mathcal{S}_t$ and $\tau_{i,t+1}=\tau_{i,t}+1$ otherwise, and the slot goes to a client of maximal staleness. Fix $j$ and a window starting at $t$, and suppose $j$ is never selected in it. For $j$ to miss the slot at round $s\ge t$, some $i_s\neq j$ with $\tau_{i_s,s}\ge\tau_{j,s}$ takes it and resets to $0$. Blocking $j$ again at $s'>s$ would need $s'-s\ge\tau_{j,t}+(s'-t)$, that is $t-s\ge\tau_{j,t}\ge 0$, which is impossible for $s\ge t$. Each of the other $N-1$ clients therefore blocks $j$ at most once, so $j$ is selected within $N-1$ rounds.}
\end{proof}
\revtwo{With $N=100$ and $T=200$ the condition is met twice over, which is why coverage holds in every run rather than on average. The cold start seeds the feedback buffer but does not create the result, and Jain's index depends mildly on it, so we report both figures.}
...
\revtwo{The query objective is not monotone on any dataset. On Fashion-MNIST it falls from $3.312$ to $0.015$ with round-to-round jumps as large as $2.672$, on CIFAR-10 it moves within $[0.015,1.023]$ with jumps to $0.703$, and on CIFAR-100 within $[0.055,3.704]$ with jumps to $3.612$. A fixed objective descended with $\eta\le 1/L_q$ cannot produce increases of this size, so the sequence is direct evidence that $q_t$ changes between rounds, which is the drift term $V_T$ of Corollary~\ref{cor:rate}. In line with Remark~\ref{rem:reading}, the meta-update magnitude in Fig.~\ref{fig:convergence}(b) settles at a small non-zero level rather than decaying to zero. We report this as evidence of drift and not as a measurement of $V_T$, which would require $q_t$ and $q_{t+1}$ at the same iterate and needs only additional logging.}

\subsection{\revision{Sensitivity and Ablation}}
\revision{These experiments test whether MAML-Select stays stable when its design choices are perturbed. We probe the cost trade-off $\lambda$, the inner-loop steps, the selector width, and the six-feature state, each on the dataset where it is most informative.}

\revision{Fig.~\ref{fig:lambda_sensitivity} reports the $\lambda$ sweep. A larger $\lambda$ weights cheaper clients more, so cost and energy fall on both datasets. Accuracy stays within about one point on Fashion-MNIST, whereas on CIFAR-10 the trade-off is sharper, falling from $64.9\%$ at $\lambda=0.1$ to $56.3\%$ at $\lambda=5$ while cost drops by about $26\%$ and energy by about $30\%$. Fairness also falls at large $\lambda$. We keep $\lambda=0.5$, which stays close to the best accuracy, lowers cost, and avoids the sharp fairness loss seen at $\lambda\ge1$.}
...
\centering
\color{black}
\caption{\revtwo{Design ablations. Left, inner-loop steps on CIFAR-10 and selector width on Fashion-MNIST, reporting accuracy and selection overhead. Right, state-feature ablation on Fashion-MNIST, where each row removes one feature. All entries are means over three seeds and defaults are shown in bold.}}
...
\begin{tabular}{@{}llrr@{}}
\toprule
\revision{Abl.} & \revision{Set.} & \revision{Acc.} & \revision{ms} \\
\midrule
\revision{Steps} & \revision{\textbf{1}} & \revision{63.8} & \revision{38.7} \\
 & \revision{2} & \revision{63.1} & \revision{39.8} \\
 & \revision{5} & \revision{62.2} & \revision{45.2} \\
\addlinespace[1pt]
\revision{$h$} & \revision{16} & \revision{90.0} & \revision{10.2} \\
 & \revision{32} & \revision{90.6} & \revision{10.4} \\
 & \revision{\textbf{64}} & \revision{90.1} & \revision{10.2} \\
 & \revision{128} & \revision{90.3} & \revision{11.6} \\
\bottomrule
\end{tabular}
\end{minipage}%
\begin{minipage}[t]{0.5\columnwidth}
...
\midrule
\textbf{Full state} & 90.23 & 122 & 0.78 \\
w/o loss & 90.16 & 123 & 0.79 \\
w/o grad.\ norm & 90.30 & 122 & 0.78 \\
w/o latency & 90.14 & 119 & 0.70 \\
w/o battery & 89.90 & 122 & 0.79 \\
w/o frequency & 90.47 & 115 & 0.65 \\
w/o staleness & 90.37 & 123 & 0.78 \\
\bottomrule
\end{tabular}
\end{minipage}
\end{table}
...
\revision{We introduced MAML-Select, an online adaptive client-selection framework for heterogeneous FL that adapts a lightweight ranking policy from recent feedback before each decision. Across Fashion-MNIST, CIFAR-10, and CIFAR-100 it achieves competitive accuracy at lower computational cost and modelled energy. \revtwo{Its score is derived from the cohort objective, and the guarantee is a tracking bound rather than convergence to a fixed point, which matches the non-stationary setting it targets.} Future work will widen the evaluation, \revtwo{measure the drift term $V_T$ directly,} and study an asynchronous extension with a differentially private treatment of the client state.}
