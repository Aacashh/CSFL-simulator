...
\begin{figure}[!t]
\centering
\includegraphics[width=0.94\columnwidth]{MAML.pdf}
...
\revtwo{Suppose every client is available each round and $\varepsilon\ge 1$. Then, after the cold start, every client is selected at least once in any window of $N$ consecutive rounds, whatever the policy $h_\phi$.}
\end{proposition}
\begin{proof}
\revtwo{Staleness obeys $\tau_{i,t+1}=0$ if $i\in\mathcal{S}_t$ and $\tau_{i,t+1}=\tau_{i,t}+1$ otherwise, and the slot goes to a client of maximal staleness. Fix $j$ and a window starting at $t$, and suppose $j$ is never selected in it. For $j$ to miss the slot at round $s\ge t$, some $i_s\neq j$ with $\tau_{i_s,s}\ge\tau_{j,s}$ takes it and resets to $0$. Since $i_s$ resets at $s$, its staleness at $s'>s$ is $s'-s-1$, so blocking $j$ again would need $s'-s-1\ge\tau_{j,t}+(s'-t)$, that is $t-s\ge\tau_{j,t}+1\ge 1$, which is impossible for $s\ge t$. Each of the other $N-1$ clients blocks $j$ at most once, so $j$ is selected within $N-1$ rounds.}
\end{proof}
